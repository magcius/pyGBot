...
from pygbot.baseplugin import BasePlugin, autoConnect
from pygbot.core import User, Channel

from pygbot.util import pretty_print_list, pp_index
from pygbot.games.fluxx.logixx import FluxxGame, FluxxPlayer
...
class Fluxx(BasePlugin):
    def __init__(self, bot, channel, config):
        super(plugin, self).__init__(bot, channel, config)
        self.users = {}

        ### xxx for right now
        self.privout = self.noteout
